    ...
    uploadImage = async (image_uri) => {
        ...
        await axios
          ...
          .then(async(res)=> {
            ...
            const url = "http://" + LocalIP + ":8888/person";
            ...
            await axios
              ...
              .then(async(res) => {
                ...
                this.captureImage()
                if (res.data.accident * 1 != 0 && this.state.people_mode==false) {
                    this.setState({people_mode:true})
                    const url2 = "http://" + ESP32IP + "/light_people"
                    axios.get(url2, {
                        headers: { "Content-Type": "application/json" },
                        timeout: 50000
                    }).then(async (res) => {
                        this.showNotification("Change Light System")
                    }).catch((error) => {
                        this.showNotification("Change Light System")
                    });
                }
